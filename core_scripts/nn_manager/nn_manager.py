    ...
    with torch.no_grad():
        ...
        for data_idx, (data_in, data_tar, data_info, idx_orig) in \
            ...
            if data_gen is None:
                ...
                for idx, seq_info in enumerate(data_info):
                    test_dataset_wrapper.putitem(data_gen_np[idx:idx+1], args.output_dir, args.output_filename_prefix, seq_info)
            ...
            for idx, seq_info in enumerate(data_info):
                if args.verbose == 2:
                    print("{:s} {:f} {:f} {:f}".format(seq_info, time_cost_load, time_cost_inf, time_cost_save))
                sample_account += 1
                _ = nii_op_display_tk.print_gen_info(seq_info, time_cost, sample_account)
...
#Con este salio sin gpu y sale bien 
"""
def f_inference_wrapper(args, pt_model, device, 
                        test_dataset_wrapper, checkpoint):

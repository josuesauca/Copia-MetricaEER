...
def f_load_checkpoint(checkpoint, args, flag_multi_device, pt_model, optimizer, 
    ...
    if args.allow_mismatched_pretrained_model:
        ...
        if type(checkpoint) is dict:
            ...
            # only model status
            #pt_model.load_state_dict(
            #    f_state_dict_wrapper(checkpoint, flag_multi_device,, strict=False))
            pt_model.load_state_dict(f_state_dict_wrapper(checkpoint, flag_multi_device, strict=False))
            nii_display.f_print("Load pretrained model")
...
def f_load_checkpoint_for_inference(checkpoint, pt_model):
    model_dict = pt_model.state_dict()
    checkpoint_dict = {k: v for k, v in checkpoint.items() if k in model_dict and model_dict[k].shape == v.shape}
